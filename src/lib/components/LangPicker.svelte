...
<div class="relative inline-flex items-center" onclick={(e) => e.stopPropagation()}>
	...
	<button onclick={toggle}
		class="flex items-center justify-center gap-1.5 rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs font-semibold text-zinc-500 transition-colors hover:text-zinc-700 cursor-pointer dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200"
	...
	{#if open}
		<div class="absolute left-1/2 top-1/2 z-10 flex -translate-x-1/2 -translate-y-1/2 gap-1 rounded-full border border-zinc-200 bg-white p-1 shadow-lg dark:border-zinc-800 dark:bg-zinc-950"
			...
			<button onclick={(e: Event) => switchTo(e, 'en')}
				class="flex items-center gap-1 rounded-full px-2.5 py-1.5 text-xs font-semibold transition-all cursor-pointer {$page.data.locale !== 'es' ? 'bg-zumito-600 text-white' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}">
