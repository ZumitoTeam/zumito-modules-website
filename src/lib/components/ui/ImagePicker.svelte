<script lang="ts">
	...
	function handleDrop(e: DragEvent) { e.preventDefault(); dragging = false; addFiles(e.dataTransfer?.files ?? null); }
	function removeFile(i: number) { URL.revokeObjectURL(selectedFiles[i].url); selectedFiles = selectedFiles.filter((_, idx) => idx !== i); }
...
<div class="space-y-3">
	<!-- Drop zone -->
	<div
		class="relative cursor-pointer rounded-2xl border-2 border-dashed p-6 text-center transition-all duration-200
	...
	>
		<!-- Actual file input (hidden via display:none but .click() works) -->
		<input type="file" {name} {accept} {multiple} class="hidden" bind:this={inputEl} onchange={onFileChange} />
		...
		{#if allPreviews.length > 0}
			<div class="grid gap-3 {multiple ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-1'}" onclick={(e: Event) => e.stopPropagation()}>
	...
	<!-- Selected file count (below the zone) -->
	{#if selectedFiles.length > 0}
		<p class="text-xs text-zinc-500">{selectedFiles.length} file{selectedFiles.length !== 1 ? 's' : ''} selected</p>
	{/if}
</div>
