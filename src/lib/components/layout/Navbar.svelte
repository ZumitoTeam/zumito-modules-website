	...
	<div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
		...
		<div class="hidden items-center gap-6 md:flex">
			...
			{#if $page.data.user}
				...
				{#if isAdmin}
					<a href="/admin" class="text-sm font-medium text-zinc-500 transition-colors hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100">Admin</a>
				{/if}
			...
			<div class="flex h-full flex-col px-5 py-6">
				...
				<div class="mt-8 grid grid-cols-2 gap-3">
					...
					{#if $page.data.user}
						...
						{#if isAdmin}
							<a href="/admin" onclick={closeMenu} class="group flex flex-col items-center gap-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-center transition-all hover:border-zumito-200 hover:bg-zumito-50 active:scale-[0.97] dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zumito-600/30 dark:hover:bg-zumito-600/5"
								in:fly={{ y: 16, duration: 250, delay: 200, easing: quartOut }}>
								<div class="flex h-12 w-12 items-center justify-center rounded-xl bg-zumito-600/10 text-zumito-600 transition-colors group-hover:bg-zumito-600 group-hover:text-white">
									<Icon icon="tabler:shield-check" class="h-6 w-6" />
								</div>
								<span class="text-sm font-semibold text-zinc-700 dark:text-zinc-200">Admin</span>
							</a>
						{/if}
					{:else}
