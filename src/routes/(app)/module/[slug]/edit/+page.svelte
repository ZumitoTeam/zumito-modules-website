<script lang="ts">
	...
	async function loadReadme() {
		...
		loadingReadme = true;
		sileo.promise(
			fetch('/api/npm/readme', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ package: pkg.trim() }) })
				.then(async (r) => { const d = await r.json(); if (!d.ok) throw new Error(d.error); return d; }),
			{
				...
				success: (d: any) => { descriptionValue = d.readme; descriptionLocked = true; if (d.repository) sourceCodeValue = d.repository; return { title: 'README loaded', description: `${pkg.trim()} description updated.`, fill: '#f0fdf4', styles: { title: 'text-green-800', description: 'text-green-600' } }; },
		...
		<form method="POST" use:enhance={onUpdate} class="mt-8 space-y-8">
			{#if activeTab === 0}
				<div class="grid gap-6 sm:grid-cols-2">
					<div>
						<div class="min-h-[4rem]"><label for="name" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Module Name <span class="text-zumito-600">*</span></label><p class="mt-1 text-xs leading-relaxed text-zinc-400">The display name shown in the marketplace.</p></div>
					...
					<div>
						<div class="min-h-[4rem]"><label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">URL Slug</label><p class="mt-1 text-xs leading-relaxed text-zinc-400">Cannot be changed after creation.</p></div>
				...
				<div class="grid gap-6 sm:grid-cols-2">
					<div>
						<div class="min-h-[4rem]"><label for="npm" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">NPM Package Name <span class="text-zumito-600">*</span></label><p class="mt-1 text-xs leading-relaxed text-zinc-400">The exact name used in <code class="rounded bg-zinc-100 px-1 py-0.5 font-mono text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">npm publish</code>.</p></div>
					...
					<div>
						<div class="min-h-[4rem]"><label for="sourceCode" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Source Code URL</label><p class="mt-1 text-xs leading-relaxed text-zinc-400">Link to the GitHub repository or source code.</p></div>
				...
				<div class="grid gap-6 sm:grid-cols-2">
					...
					<div>
						<div class="min-h-[4rem]"><label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Features</label><p class="mt-1 text-xs leading-relaxed text-zinc-400">Existing tags to categorize your module. You can add new ones.</p></div>
						<div class="mt-2 flex flex-wrap gap-1.5">
							{#each data.features as f}
								<label class="cursor-pointer rounded-full border px-3 py-1.5 text-xs transition-all has-[:checked]:border-zumito-600 has-[:checked]:bg-zumito-50 has-[:checked]:text-zumito-700 dark:border-zinc-800 dark:has-[:checked]:border-zumito-600 dark:has-[:checked]:bg-zumito-600/10 dark:has-[:checked]:text-zumito-400"
									...
									<input type="checkbox" name="features" value={f.name} checked={data.mod.features?.some((mf: any) => mf.name === f.name)} class="sr-only" />{f.emoji} {f.name}
								</label>
							{/each}
						</div>
				...
				{#if data.modules.length > 0}
					<div>
						...
						<select name="dependencies" multiple class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm text-zinc-900 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100" size={6}>
							{#each data.modules as m}<option value={m.id} selected={data.mod.dependencies?.some((d: any) => d.dependencyId === m.id)}>{m.name}</option>{/each}
						</select>
					</div>
				{/if}
			{/if}
				...
				<div>
					<div class="flex items-center justify-between">
						...
						<button type="button" onclick={loadReadme} disabled={loadingReadme || !npmValue.trim()} class="flex items-center gap-1.5 rounded-full border border-zinc-300 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 transition-all hover:border-zumito-300 hover:text-zumito-600 disabled:opacity-50 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-400 dark:hover:border-zumito-600/40">
							{#if loadingReadme}<svg class="h-3.5 w-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>{/if}Load from npm
						</button>
					</div>
					{#if descriptionLocked}
						<div class="mt-2 rounded-xl border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-950">
							<div class="flex items-center justify-between mb-2"><span class="text-xs font-medium text-green-700 dark:text-green-400">Loaded from npm README</span><button type="button" onclick={() => descriptionLocked = false} class="text-xs text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors">Unlock to edit</button></div>
