<script lang="ts">
    ...
    $: if (form?.status === 200 && form?.module) {
        goto(`/module/${form.module.name}`);
    }
</script>

<form use:enhance action="?/editModule" method="POST" class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
    <div class="flex flex-col gap-5 w-full">
          ...
          <div class="max-w-2xl">
              ...
              <input type="text" id="name" name="name" on:keyup={formatName} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-pink-500 focus:border-pink-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-pink-500 dark:focus:border-pink-500" placeholder="zumito-framework" required bind:value={name} />
          </div>
          <div>
          ...
          <!--<div>
              ...
              <FilePond bind:this={pond} {filepondName}
              server="/api"
              allowMultiple={true}
              oninit={handleInit}
          ...
          <button type="submit" class="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold py-2 px-4 rounded">Save Changes</button>
