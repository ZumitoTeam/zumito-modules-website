				...
				<form method="POST" use:enhance class="mt-8 space-y-8">
					...
					{#if activeTab === 0}
						<div class="grid gap-6 sm:grid-cols-2">
							<div>
								<div class="min-h-[4rem]">
									<label for="name" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Module Name <span class="text-zumito-600">*</span></label>
									<p class="mt-1 text-xs leading-relaxed text-zinc-400">The display name shown in the marketplace.</p>
								</div>
								<input id="name" name="name" required placeholder="My Awesome Command Module" bind:value={nameValue} oninput={generateSlug} class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-600" />
							</div>
							<div>
								<div class="min-h-[4rem]">
									<label for="slug" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">URL Slug <span class="text-zumito-600">*</span></label>
									<p class="mt-1 text-xs leading-relaxed text-zinc-400">Auto-generated from name. Use lowercase letters and dashes.</p>
								</div>
								<input id="slug" name="slug" required placeholder="my-awesome-command-module" value={slugValue} oninput={onSlugInput} class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm font-mono text-zinc-900 placeholder:text-zinc-400 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-600" />
							</div>
						</div>
						<div>
						...
						<div>
							...
							<textarea id="instructions" name="instructions" rows={5} bind:value={instructionsValue} placeholder={`## Installation\n\n1. Install the module:\n\`\`\`bash\nnpm install @zumito-modules/my-module\n\`\`\`\n2. Add to your \`zumito.config.ts\` bundles array.\n3. Restart your bot.`} class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 text-sm font-mono text-zinc-900 leading-relaxed placeholder:text-zinc-400 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-600"></textarea>
						</div>
						<div class="grid gap-6 sm:grid-cols-2">
							<div>
								<div class="min-h-[4rem]">
								...
								<input id="npm" name="npm" required placeholder="@zumito-modules/music-queue" bind:value={npmValue} class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm font-mono text-zinc-900 placeholder:text-zinc-400 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-600" />
							</div>
							<div>
								<div class="min-h-[4rem]">
								...
								<input id="sourceCode" name="sourceCode" placeholder="https://github.com/username/my-module" bind:value={sourceCodeValue} class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm font-mono text-zinc-900 placeholder:text-zinc-400 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-600" />
							</div>
						</div>
						<div class="grid gap-6 sm:grid-cols-2">
							<div>
								<div class="min-h-[4rem]"><label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Pricing</label></div>
								...
								<div class="mt-2 flex flex-wrap gap-1.5">
									{#each data.features as f}
										<label class="cursor-pointer rounded-full border border-zinc-200 px-3 py-1.5 text-xs transition-all has-[:checked]:border-zumito-600 has-[:checked]:bg-zumito-50 has-[:checked]:text-zumito-700 dark:border-zinc-800 dark:has-[:checked]:border-zumito-600 dark:has-[:checked]:bg-zumito-600/10 dark:has-[:checked]:text-zumito-400">
											<input type="checkbox" name="features" value={f.name} class="sr-only" />{f.emoji} {f.name}
										</label>
									{/each}
								</div>
						...
						{#if data.modules.length > 0}
							<div>
								...
								<select name="dependencies" multiple class="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm text-zinc-900 transition-colors focus:border-zumito-500 focus:ring-1 focus:ring-zumito-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100" size={6}>
									{#each data.modules as m}<option value={m.id}>{m.name}</option>{/each}
								</select>
							</div>
						{/if}
					{/if}
					...
					{#if activeTab === 1}
						<div>
							<div class="flex items-center justify-between">
								<label for="description" class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Description <span class="text-zumito-600">*</span></label>
								<button type="button" onclick={loadReadme} disabled={loadingReadme || !npmValue.trim()} class="flex items-center gap-1.5 rounded-full border border-zinc-300 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 transition-all hover:border-zumito-300 hover:text-zumito-600 disabled:opacity-50 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-400 dark:hover:border-zumito-600/40">
									{#if loadingReadme}<svg class="h-3.5 w-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>{/if}
									Load from npm
								</button>
							</div>
					...
					{#if activeTab === 2}
						<div class="flex flex-col items-center gap-4 py-12 text-center">
							<div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-zumito-50 text-zumito-600 dark:bg-zumito-600/10">
							...
							<h3 class="text-lg font-semibold text-zinc-500">Images coming soon</h3>
							<p class="max-w-sm text-sm text-zinc-400">You'll be able to upload screenshots and an icon for your module. For now, you can add them after publishing via the Edit page.</p>
						</div>
					{/if}
					...
					{#if activeTab === 3}
						<div>
							<div class="flex items-center justify-between">
								<label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300">Frequently Asked Questions</label>
								<button type="button" onclick={addFaq} class="flex items-center gap-1 rounded-full border border-zinc-300 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 transition-all hover:border-zumito-300 hover:text-zumito-600 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-400">
									<svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg> Add FAQ
								</button>
							</div>
							<div class="mt-4 space-y-4">
								{#each faqItems as faq, i}
									<div class="relative rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900">
										<button type="button" onclick={() => removeFaq(i)} class="absolute right-3 top-3 text-zinc-400 hover:text-red-500 transition-colors">
								...
								{#if faqItems.length === 0}
									<p class="text-sm text-zinc-400 text-center py-8">No FAQs added yet. Click "Add FAQ" to get started.</p>
								{/if}
							</div>
						</div>
					{/if}
