			...
			<section class="mt-8 rounded-2xl border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-950">
				...
				<form method="POST" use:enhance enctype="multipart/form-data" class="mt-8 space-y-8">
					...
					<div class="transition-all duration-200 {activeTab === 0 ? 'relative' : 'absolute inset-0 invisible opacity-0 pointer-events-none'}">
						...
						{#if data.modules.length > 0}<div class="mt-8"><DependencyPicker available={data.modules} /></div>
						<div class="mt-8"><DependencyPicker available={data.modules} name="addons" label="Addons" /></div>{/if}
					</div>
					...
					<div class="flex items-center gap-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
						<button type="submit" class="rounded-full bg-zumito-600 px-8 py-3 text-sm font-semibold text-white transition-all hover:bg-zumito-700 active:scale-[0.98] cursor-pointer">Publish Module</button>
