...
<Background>
    ...
    <Container class="pb-12 pt-8">
        <div class="mb-12 text-center">
            ...
            <div class="mb-8 flex justify-center gap-4">
                <a href="/modules" class="rounded-lg bg-pink-600 px-8 py-3 font-semibold text-white transition-colors hover:bg-pink-700">
                    Explore Modules
                </a>
                <a href="/submit" class="rounded-lg border border-pink-600 px-8 py-3 font-semibold text-pink-600 transition-colors hover:bg-pink-50">
                    Submit Module
                </a>
            </div>
        </div>
    ...
    <Container class="mb-16">
        <div class="mb-8 text-center">
            <h2 class="mb-4 text-3xl font-bold text-gray-800">Popular Features</h2>
            <p class="mx-auto max-w-2xl text-gray-600">Explore modules by their most popular features and capabilities</p>
        </div>
        <div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
            {#each featuredFeatures as feature}
                <a href="/modules?features={feature.name}" 
                   class="group relative overflow-hidden rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-4 text-center transition-all duration-300 hover:scale-105 hover:border-pink-200 hover:from-pink-50 hover:to-white hover:shadow-lg">
                    <div class="relative z-10">
                        <div class="mb-3 text-3xl transition-transform duration-300 group-hover:rotate-6 group-hover:scale-110">
                            {feature.icon}
                        </div>
                        <div class="mb-2 text-sm font-semibold text-gray-800 transition-colors group-hover:text-pink-700">
                            {feature.name}
                        </div>
                        <div class="inline-flex items-center rounded-full bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600 transition-all group-hover:bg-pink-100 group-hover:text-pink-700">
                            {feature.count} modules
                        </div>
                    </div>
                    
                    <!-- Hover effect overlay -->
                    <div class="absolute inset-0 rounded-xl bg-gradient-to-br from-pink-500/5 to-purple-500/5 opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
                    
                    <!-- Decorative elements -->
                    <div class="absolute -right-2 -top-2 h-8 w-8 rounded-full bg-pink-100 opacity-0 transition-all duration-300 group-hover:scale-100 group-hover:opacity-100"></div>
                    <div class="absolute -bottom-1 -left-1 h-6 w-6 rounded-full bg-purple-100 opacity-0 transition-all duration-300 group-hover:opacity-100"></div>
                </a>
            {/each}
        </div>
        ...
        <div class="mt-8 text-center">
            <a href="/modules" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-6 py-3 font-medium text-gray-700 transition-all duration-200 hover:border-pink-300 hover:bg-pink-50 hover:text-pink-700">
    ...
    <Container class="mb-12">
        <div class="mx-auto max-w-4xl">
            <div class="mb-6 text-center">
                <h2 class="mb-2 text-2xl font-bold text-gray-800">Find Your Perfect Module</h2>
                <p class="text-gray-600">Search through our collection of powerful modules</p>
            </div>
            ...
            <form on:submit|preventDefault={() => handleSearch()}>
                <div class="relative">
                    ...
                    <input 
                        ...
                        class="w-full rounded-xl border-2 border-gray-200 bg-white py-4 pl-14 pr-32 text-lg shadow-sm transition-all duration-200 hover:border-gray-300 focus:border-pink-500 focus:outline-none focus:ring-4 focus:ring-pink-100"
                    />
                    <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                        <button 
                            type="submit"
                            class="flex items-center gap-2 rounded-lg bg-pink-600 px-6 py-2.5 font-semibold text-white transition-all duration-200 hover:scale-105 hover:bg-pink-700 focus:outline-none focus:ring-4 focus:ring-pink-200">
            ...
            <div class="mt-4 flex flex-wrap justify-center gap-2">
                ...
                {#each ['API', 'Database', 'UI Components', 'Auth', 'Analytics'] as suggestion}
                    <button 
                        ...
                        class="rounded-full bg-gray-100 px-3 py-1 text-sm text-gray-700 transition-colors hover:bg-pink-100 hover:text-pink-700">
                        {suggestion}
                    </button>
                {/each}
            </div>
    ...
    <Container class="pb-16">
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-gray-600">Discover the newest additions to our module collection</p>
            </div>
            <a href="/modules" class="flex items-center gap-2 rounded-lg bg-pink-50 px-4 py-2 font-semibold text-pink-600 transition-colors hover:bg-pink-100 hover:text-pink-700">
                View All 
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {#each modules as module}
                <article class="group overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-pink-200 hover:shadow-lg">
                            ...
                            <div class="mb-4 flex items-start gap-4">
                                <div class="relative">
                                    <img src="{(module.icon && (PUBLIC_MINIO_URL + module.icon)) || placeholderImage}" 
                                         ...
                                         class="h-16 w-16 rounded-xl border border-gray-200 object-cover shadow-sm transition-transform group-hover:scale-105">
                                    <div class="absolute -bottom-1 -right-1 rounded-full bg-green-100 p-1">
                                        <div class="h-2 w-2 rounded-full bg-green-500"></div>
                                    </div>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <h3 class="truncate text-lg font-bold text-gray-900 transition-colors group-hover:text-pink-600">
                                        {module.name}
                                    </h3>
                                    <p class="mb-2 text-sm text-gray-500">
                                        by <span class="font-medium">{module.author?.username}</span>
                                    </p>
                                    <div class="line-clamp-2 text-sm leading-relaxed text-gray-600">
                            ...
                            {#if module.features && module.features.length > 0}
                                <div class="mb-4 flex flex-wrap gap-1">
                                    {#each module.features.slice(0, 3) as feature}
                                        <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600 transition-colors group-hover:bg-pink-50 group-hover:text-pink-700">
                                            {feature.name}
                                        </span>
                                    {/each}
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4 text-sm text-gray-600">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="text-gray-500">installs</span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex h-8 w-8 items-center justify-center rounded-full bg-pink-100 text-pink-600 transition-colors group-hover:bg-pink-200">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                        </svg>
                                    </div>
        ...
        <div class="mt-12 text-center">
            <p class="mb-6 text-gray-600">Can't find what you're looking for?</p>
            <div class="flex justify-center gap-4">
                <a href="/modules" class="rounded-lg bg-pink-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-pink-700">
                    Browse All Modules
                </a>
                <a href="/submit" class="rounded-lg border border-pink-600 px-6 py-3 font-semibold text-pink-600 transition-colors hover:bg-pink-50">
                    Submit Your Module
                </a>
            </div>
        </div>
