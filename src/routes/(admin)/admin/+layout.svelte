			...
			<div class="flex items-center">
				{#each nav as item}
					<a href={item.href}
						...
						{isActive(item.href) ? 'text-zumito-600 before:absolute before:inset-x-4 before:bottom-0 before:h-0.5 before:rounded-full before:bg-zumito-600' : 'text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200'}">
